defmodule SeekTest do
  ...
  setup_all do
    classic  = Product.new(sku: "classic", name: "Classic Ad", price: 269_99)
    standout = Product.new(sku: "standout", name: "Standout Ad", price: 322_99)
    premium  = Product.new(sku: "premium", name: "Premium Ad", price: 394_99)

    unilever = Customer.new(slug: "unilever", name: "Unilever")
    apple    = Customer.new(slug: "apple", name: "Apple")
    nike     = Customer.new(slug: "nike", name: "Nike")
    ford     = Customer.new(slug: "ford", name: "Ford")
  ...
  test "Checkout [classic, classic, premium] for unilever", state do
    checkout =
      state[:checkout]
      |> Checkout.for_customer(state[:customers][:unilever])
      |> Checkout.add_item(state[:products][:classic])
      |> Checkout.add_item(state[:products][:classic])
      |> Checkout.add_item(state[:products][:classic])
      |> Checkout.add_item(state[:products][:premium])
  ...
  test "Checkout [standout, standout, standout, premium] for apple", state do
    checkout =
      state[:checkout]
      |> Checkout.for_customer(state[:customers][:apple])
      |> Checkout.add_item(state[:products][:standout])
      |> Checkout.add_item(state[:products][:standout])
      |> Checkout.add_item(state[:products][:standout])
      |> Checkout.add_item(state[:products][:premium])
  ...
  test "Checkout [premium, premium, premium, premium] for nike", state do
    checkout =
      state[:checkout]
      |> Checkout.for_customer(state[:customers][:nike])
      |> Checkout.add_item(state[:products][:premium])
      |> Checkout.add_item(state[:products][:premium])
      |> Checkout.add_item(state[:products][:premium])
      |> Checkout.add_item(state[:products][:premium])
  ...
  test "Checkout [classic, classic, classic, classic, classic, standout, premium, premium, premium] for ford", state do
    checkout =
      state[:checkout]
      |> Checkout.for_customer(state[:customers][:ford])
      |> Checkout.add_item(state[:products][:classic])
      |> Checkout.add_item(state[:products][:classic])
      |> Checkout.add_item(state[:products][:classic])
      |> Checkout.add_item(state[:products][:classic])
